...
import { Users, ChevronRight, User2, Mail, Phone, Briefcase, MonitorCog } from "lucide-react";
...
export default function ItDepartment() {
  ...
  const itTeam = teams.find((t: any) => {
    const name = t.name?.toLowerCase();
    return name?.includes("it") || name?.includes("maintenance");
  });

  const members = (itTeam?.users || []).filter((m: any) => {
  ...
  return (
    <div className="space-y-6">
      <PrivatePageHeading
        icon={MonitorCog}
        title="IT Maintenance Team"
        description="All members of the IT Maintenance team"
      />

      <StatsCardGrid
        items={[
          { label: "Total Members", value: itTeam?.users?.length || 0, icon: Users },
          { label: "Active Members", value: itTeam?.users?.filter((u: any) => u.isActive).length || 0, icon: User2 },
          { label: "Inactive Members", value: itTeam?.users?.filter((u: any) => !u.isActive).length || 0, icon: User2 },
        ]}
        columns={3}
        size="lg"
      />
      ...
      <div className="bg-white rounded-2xl border border-brand-neutral-border shadow-sm p-4">
        <input
          ...
          className="w-full max-w-sm px-3 py-2 text-sm border border-brand-neutral-border rounded-lg focus:outline-none focus:ring-2 focus:ring-brand-primary"
        />
      </div>

      <TableWraper title="IT Team Members" variant="brand">
        {!itTeam ? (
          <div className="p-12 text-center">
            <Users size={28} className="text-brand-neutral-muted mx-auto mb-3" />
            <p className="text-brand-neutral-muted text-sm">IT team not found.</p>
          </div>
        ) : members.length === 0 ? (
          <div className="p-12 text-center">
            <Users size={28} className="text-brand-neutral-muted mx-auto mb-3" />
                ...
                {members.map((member: any) => {
                  ...
                  return (
                    <tr key={member.id} className="hover:bg-brand-neutral-light/50 transition-colors">
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-3">
                          {src ? (
                            ...
                            <div className="w-9 h-9 rounded-full bg-rose-100 flex items-center justify-center text-rose-700 shrink-0 font-bold text-xs">
                      ...
                      <td className="px-4 py-3">
                        <span className="text-[10px] bg-rose-50 text-rose-700 px-2 py-0.5 rounded-full font-bold capitalize">
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center justify-center gap-2">
                          <Link
                            ...
                            className="text-[11px] font-medium bg-rose-50 border border-rose-200 text-rose-700 px-3 py-1.5 rounded-lg hover:bg-rose-100 transition inline-flex items-center gap-1"
                          >
                            <Briefcase size={11} /> View Details <ChevronRight size={11} />
                          </Link>
                        </div>
                      </td>
                    </tr>
